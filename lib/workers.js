...
workers.alertUser = function (checkData) {
    ...
    helpers.sendSms(checkData.phone, message, (err) => {
        if (!err) {
            debug('User was alerted through sms: ' + message);
            return;
        }

        debug('Could not send alert to user who had a state change in their check');
    });
};
        ...
        if (!err && logs && logs.length > 0) {
            logs.forEach((log) => {
                ...
                _logs.compress(logId, newFileId, (err) => {
                    ...
                    debug('Could not compress log file:', err);
                });
            });
            return;
        }
